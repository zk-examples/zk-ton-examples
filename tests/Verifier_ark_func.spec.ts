...
describe('Verifier_ark_func', () => {
    let code: Cell;
    let GAS_LOG = new GasLogAndSave('Verifier_ark_func');

    beforeAll(async () => {
        code = await compile('Verifier_ark_func');
        GAS_LOG.rememberBocSize('Verifier_ark_func', code);
    });
