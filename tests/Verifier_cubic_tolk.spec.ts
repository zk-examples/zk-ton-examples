...
describe('Verifier_cubic_tolk', () => {
    let code: Cell;
    let GAS_LOG = new GasLogAndSave('Verifier_cubic_tolk');

    beforeAll(async () => {
        code = await compile('Verifier_cubic_tolk');
        GAS_LOG.rememberBocSize('Verifier_cubic_tolk', code);
    });
    ...
    it('should verify', async () => {
        ...
        expect(await verifier.getVerifyCubic({ pi_a, pi_b, pi_c, pubInputs })).toBe(true);
