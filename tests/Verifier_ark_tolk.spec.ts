...
describe('Verifier_ark_tolk', () => {
    let code: Cell;
    let GAS_LOG = new GasLogAndSave('Verifier_ark_tolk');

    beforeAll(async () => {
        code = await compile('Verifier_ark_tolk');
        GAS_LOG.rememberBocSize('Verifier_ark_tolk', code);
    });
